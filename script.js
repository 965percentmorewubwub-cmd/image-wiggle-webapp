...
const jellyState = { x: 0, y: 0, vx: 0, vy: 0, impulseClock: 0, seed: 0 };
...
function updateJellyState(dt) {
  ...
  // speed: higher = quicker vibration + more frequent impulses
  const naturalFreq = 5 + speed * 0.22;
  const springK = naturalFreq * naturalFreq;
  const damping = 2.6 + (40 - speed) * 0.05;

  jellyState.impulseClock += dt;
  const impulseInterval = Math.max(0.12, 0.62 - speed * 0.011);
  if (jellyState.impulseClock >= impulseInterval) {
    jellyState.impulseClock = 0;
    jellyState.seed += 1;
    const angle = jellyState.seed * 1.7;
    const impulse = (strength / 40) * (10 + speed * 0.2);
    jellyState.vx += Math.cos(angle) * impulse;
    jellyState.vy += Math.sin(angle * 1.13) * impulse;
  }

  const ax = -springK * jellyState.x - damping * jellyState.vx;
  const ay = -springK * jellyState.y - damping * jellyState.vy;
  jellyState.vx += ax * dt;
  jellyState.vy += ay * dt;
  jellyState.x += jellyState.vx * dt;
  jellyState.y += jellyState.vy * dt;
}

function computeJellyOffset(x, y) {
  const dx = x - maskCentroid.x;
  const dy = y - maskCentroid.y;
  const dist = Math.hypot(dx, dy);
  const nr = Math.min(1, dist / (maskRadius + 0.0001));

  // center moves more, boundary moves less
  const local = Math.pow(1 - nr, 1.8);
  ...
  const baseX = jellyState.x * weight;
  const baseY = jellyState.y * weight;

  // slight squash/stretch along displacement direction for jelly feel
  const proj = (dirX * jellyState.x + dirY * jellyState.y) * 0.35 * weight;
...
function renderDeformedFrame() {
  ...
  for (let y = minY; y <= maxY; y++) {
    for (let x = minX; x <= maxX; x++) {
      ...
      const { ox, oy } = computeJellyOffset(x, y);
...
function renderFrame(now) {
  ...
  const dt = Math.min(0.035, Math.max(0.001, (now - lastFrameTime) / 1000));
  lastFrameTime = now;
  updateJellyState(dt);
  renderDeformedFrame();
